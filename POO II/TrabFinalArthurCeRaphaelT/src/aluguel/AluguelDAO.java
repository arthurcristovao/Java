...
public class AluguelDAO {
    ...
    public ArrayList<Aluguel> listByCliente(int clienteId) {
        ArrayList<Aluguel> alugueisList = new ArrayList<>();

        try {
            Connection conn = ConexaoMySQL.getConexaoMySQL();
            PreparedStatement ps = conn.prepareStatement("SELECT idAluguel, carro_id, dataAluguel, dataDevolucao FROM Aluguel WHERE cliente_id = ?");
    ...
    public ArrayList<Aluguel> listByCarro(int carroId) {
        ArrayList<Aluguel> alugueisList = new ArrayList<>();

        try {
            Connection conn = ConexaoMySQL.getConexaoMySQL();
            PreparedStatement ps = conn.prepareStatement("SELECT idAluguel, cliente_id, dataAluguel, dataDevolucao FROM Aluguel WHERE carro_id = ?");
            ps.setInt(1, carroId);
            ResultSet rs = ps.executeQuery();
