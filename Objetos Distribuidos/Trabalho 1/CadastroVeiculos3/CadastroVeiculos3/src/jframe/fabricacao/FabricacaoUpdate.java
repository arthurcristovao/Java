 ...
 * @author Arthur
 */
public class FabricacaoUpdate extends javax.swing.JFrame {
    private Fabricacao fabricacaoSelecionada;
    /**
     * Creates new form veiculoRead
     */
    public FabricacaoUpdate(Fabricacao fabricacaoSelecionada) {
        this.fabricacaoSelecionada = fabricacaoSelecionada;
        initComponents();
        listarVeiculos();
        listarFabricantes();
    ...
    private void preencherFabricacao(){
        Thread thread = new Thread(() -> {
            campoDataFabricacao.setText(fabricacaoSelecionada.getDataFabricacao().format(DateTimeFormatter.ofPattern("dd/MM/yyyy")));
            campoPaisFabricacao.setText(fabricacaoSelecionada.getPaisFabricacao());

            // Seleciona o veículo na tabela veiculoRead
            for (int i = 0; i < veiculoRead.getRowCount(); i++) {
                int idVeiculoTabela = (int) veiculoRead.getValueAt(i, 0);
                if (idVeiculoTabela == fabricacaoSelecionada.getVeiculo().getIdVeiculo()) {
                    veiculoRead.setRowSelectionInterval(i, i);
                    veiculoRead.scrollRectToVisible(veiculoRead.getCellRect(i, 0, true));
                    break;
                }
            }

            // Seleciona o fabricante na tabela fabricanteRead
            for (int i = 0; i < fabricanteRead.getRowCount(); i++) {
                int idFabricanteTabela = (int) fabricanteRead.getValueAt(i, 0);
                if (idFabricanteTabela == fabricacaoSelecionada.getFabricante().getIdFabricante()) {
                    fabricanteRead.setRowSelectionInterval(i, i);
                    fabricanteRead.scrollRectToVisible(fabricanteRead.getCellRect(i, 0, true));
                    break;
                }
            }
        });
    ...
    private void initComponents() {
        ...
        jLabel3.setText("Edite uma fabricação");
    ...
    private void EditarActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_EditarActionPerformed
        Thread thread = new Thread(() -> {
            ...
            // Verifica se um item de cada tabela está selecionado
            ...
            // Validação do campo de país de fabricação
            ...
            // Obtém os IDs selecionados nas tabelas
            ...
            // Monta o objeto Veiculo com os dados da tabela
            ...
            // Monta o objeto Fabricante com os dados da tabela
            ...
            // Cria a data de fabricação a partir da string fornecida
            DateTimeFormatter formatter = DateTimeFormatter.ofPattern("dd/MM/yyyy");
            LocalDate dataFabricacaoObj = LocalDate.parse(dataFabricacao, formatter);

            // Cria o objeto Fabricacao com os objetos Veiculo e Fabricante montados
            Fabricacao fabricacaoAtualizada = new Fabricacao(fabricacaoSelecionada.getIdFabricacao(),veiculoSelecionado, fabricanteSelecionado, dataFabricacaoObj, paisFabricacao);

        
            // Insere o objeto Fabricacao no banco de dados
            FabricacaoDAO fabricacaoDAO = new FabricacaoDAO();
            int rowCount = fabricacaoDAO.update(fabricacaoAtualizada);

            // Exibe mensagem de sucesso ou erro
            SwingUtilities.invokeLater(() -> {
                if (rowCount > 0) {
                    JOptionPane.showMessageDialog(this, "Fabricação editada com sucesso!");
                    setVisible(false);
                    new FabricacaoRead().setVisible(true);
                } else {
                    JOptionPane.showMessageDialog(this, "Erro ao editar a fabricação.");
                }
            });
        });
        thread.start();
